...
'tempformat' = 'Celcius'
  ...
  for k in range(nrs):
    ...
    syield0,dsi,Yc,Lc=mtdb4hard(Yp,Lp,toler,dtime[k],temp[k],V)
    syield0fd,dsifd,Ycfd,Lcfd=mtdb4hard(Yp,Lp,toler+toler,dtime[k],temp[k],V)
	...
	syield,dsiA,Yc,Lc=mtdb4hard(Yp,Lp,deqpl,dtime[k],temp[k],V)
	syieldfd,dsifd,Ycfd,Lcfd=mtdb4hard(Yp,Lp,deqpl+toler,dtime[k],temp[k],V)	
...
# ---------------------------------------------------------------- 
# ----------------------------------------------------------------
def mtdb4hard(Yp,Lp,depl,dtime,temp,props):
#
  keynames = ['mu0','d0','t0','sa','s0','si','a0e','C0','cD','r','C1','C20','a02','a0i','rate0','rate02','qe','pe','qi','pi','r0','beta','U0']
  [mu0,d0,t0,sa,s0,si,a0e,C0,cD,r,C1,C20,a02,a0i,rate0,rate02,qe,pe,qi,pi,r0,beta,U0] = [props[kn] for kn in keynames]
  toler = 1e-8
#     check 
  depl = np.abs(depl)
  rate = depl/dtime
...
# d(Yc)/d(epl)
  dYc = (dYde-depl*dC2de*Yc+depl*cD*r*Lc**(r-1.))/dFdY
