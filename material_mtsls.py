...
'tempformat' = 'Celcius'
  ...
  for k in range(nrs):
    ...
    syield0,dsi,sec,slc=mtshard4(sep,slp,toler,dtime[k],temp[k],V)
    sec = sep
	...
	syield,dsi,sec,slc=mtshard4(sep,slp,toler,dtime[k],temp[k],V)
      if np.abs(f)>toler:
...
# ---------------------------------------------------------------- 
# ----------------------------------------------------------------
def mtshard4(sep,slp,depl,dtime,temp,props):
#
  keynames = ['mu0','d0','t0','sa','s0es','si','a0e','a0es','a0i','hard0','rate0','rate0es','qe','pe','qi','pi','cD','r']
  [mu0,d0,t0,sa,s0es,si,a0e,a0es,a0i,hard0,rate0,rate0es,qe,pe,qi,pi,cD,r] = [props[kn] for kn in keynames]
  toler = 1e-8
  #cmname = props['model']
#     check 
  depl = np.abs(depl)
...
#
  ...
  Xp = np.array([sep,slp])
#
  harde = hard0*(1.-sec/sat)
  dhardede = -depl*hard0/sat
  dhardedl = depl*hard0*sec
  hardl = cD*slc**(1.-1./r)
  dhardldl = (1.-1./r)*cD*slc**(-1./r)
  
  if sec > 0:
    harde = hard0*(1. - sec/sat + slc/sec)
    dhardede = depl*hard0*(-1./sat-slc/(sec**2.))
  
  A = np.array([[dhardede,dhardedl],[0.,dhardldl]])
  
  f = np.array([harde,hardl])*depl
  
  ff = np.sqrt(np.sum(f*f))
  counter = 0.
  while (np.abs(ff)>toler)&(counter<10):
    counter+=1
    X = X - np.linalg.solve(A,X)
    sec,slc = X[0],X[1]
    harde = hard0*(1. - sec/sat + slc/sec)
    hardl = cD*slc**(1.-1./r)
    f = X-Xp-np.array([harde,hardl])*depl
    ff = np.sqrt(np.sum(f*f))
    A = np.array([[depl*hard0*(-1./sat-slc/(sec**2.)),depl*hard0*sec],
                  [0.,(1.-1./r)*cD*slc**(-1./r)]])
    print "|F| :",ff
    print "xe,xl:: ",sec,slc
  if np.abs(ff)>toler:
